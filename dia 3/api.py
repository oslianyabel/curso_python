        ...
        try:
            if request.method in ["POST", "PUT"]:
                ...
                if "name" not in user_data or not isinstance(user_data["name"], str):
                    return jsonify(
                        {"Error": "El campo 'name' es requerido y debe ser string"}
                    ), 400

                if "age" not in user_data or not isinstance(user_data["age"], int):
                    return jsonify(
                        {"Error": "El campo 'age' es requerido y debe ser entero"}
                    ), 400

            return func(*args, **kwargs)

        except Exception as exc:
            logger.error(f"Error durante la validación de los datos: {str(exc)}")
            return jsonify({"Error": "Error validando datos"}), 400
